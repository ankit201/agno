    ...
    try:
        if hasattr(vector_db, "delete_collection"):
            ...
            if keys:
                ...
                print(
                    f"✅ Removed {len(keys)} keys from Redis collection: {collection_name}"
                )
...
def main():
    ...
    print(
        f"✅ Created {len(populate_questions)} individual text files for vector store population"
    )
    ...
    cleanup_response = (
        input("\n🗑️  Do you want to clean up files and database? (y/n): ")
        .lower()
        .strip()
    )

...
class RedisVL(VectorDb):
    ...
    async def async_insert(self, documents: List[Document], filters: Optional[Dict[str, Any]] = None, batch_size: int = 100) -> None:
    ...
    def search(self, query: str, limit: int = 5, search_type: Optional[SearchType] = None, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
    ...
    def _vector_search(self, query: str, limit: int, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        try:
            vector_query = VectorQuery(
                vector=self._format_embedding(query_embedding),
    ...
    def _keyword_search(self, query: str, limit: int, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        try:
            ...
            search_query = f"@content:{query}" if query != "*" else "*"
    ...
    def _process_search_results(self, results: List[Dict[str, Any]], query: str) -> List[Document]:
        ...
        if hasattr(results, 'docs'):
            ...
            try:
                ...
                log_debug(f"Unable to process search results format: {type(results)}")
                return []
    ...
    async def async_search(self, query: str, limit: int = 5, search_type: Optional[SearchType] = None, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
    ...
    def get_count(self) -> int:
        ...
        try:
            if self.exists():
                ...
                results = self.index.search("*", limit=1)
                return results.total if hasattr(results, 'total') else 0
        except Exception as e:
